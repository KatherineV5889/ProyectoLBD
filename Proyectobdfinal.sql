...
--estas son para el trigger de tablas temporales YA
...
-- Crear la tabla temporal tabla_temporal_2 YA
...
CREATE TABLE procesos (
    ...
    fecha_fin DATE,                          
    usuario_ejecucion VARCHAR2(100),         
...
DECLARE
    v_lista_productos VARCHAR2(4000);
BEGIN
    v_lista_productos := gestionar_productos.listar_productos_por_proveedor(4);
    DBMS_OUTPUT.PUT_LINE(v_lista_productos);
END;
...
DECLARE
    v_datos_cliente VARCHAR2(500);
BEGIN
    v_datos_cliente := gestionar_clientes.obtener_datos_cliente(1); -- ID de cliente cambiarlo si quiere ver otro id
...
--Procedimiento que nos ayuda a actualizar la dirreccion Ojo tiene que descativar el trigger asntes de correrlo el de auditoria cambios
BEGIN
...
BEGIN
    ...
    FOR v_precio IN (SELECT precio_producto FROM productos WHERE id_producto = 24) LOOP
...
CREATE OR REPLACE PACKAGE BODY gestionar_inventarios AS
    ...
    PROCEDURE listar_inventarios_bajos IS
        ...
        v_inventario c_inventarios%ROWTYPE;
    BEGIN
        OPEN c_inventarios;
        LOOP
...
CREATE OR REPLACE PACKAGE gestionar_empleados AS
    ...
    FUNCTION listar_nombres_empleados_tienda(p_id_tienda NUMBER) RETURN SYS_REFCURSOR;
...
DECLARE
    ...
    c_empleados SYS_REFCURSOR;
BEGIN
    c_empleados := gestionar_empleados.listar_nombres_empleados_tienda(1);
    LOOP
...
BEGIN
    v_info_empleado := gestionar_empleados.obtener_empleado(1);
    DBMS_OUTPUT.PUT_LINE(v_info_empleado);
END;

--Procedimiento gestionar empleados--
BEGIN
    gestionar_empleados.asignar_empleado_tienda(1, 2);
...
CREATE OR REPLACE PACKAGE gestionar_proveedores AS
    ...
    PROCEDURE listar_proveedores_por_ciudad(p_ciudad VARCHAR2);
...
DECLARE
    v_info VARCHAR2(200);
BEGIN
    v_info := gestionar_proveedores.obtener_info_proveedor(1);
    DBMS_OUTPUT.PUT_LINE('Información del proveedor: ' || v_info);
END;

--Procedimiento para actualizar contacto --
BEGIN
    gestionar_proveedores.actualizar_telefono_proveedor(1, '89675412');
...
CREATE OR REPLACE PACKAGE analizar_ventas AS
    ...
    TYPE cursor_ventas IS REF CURSOR RETURN ventas%ROWTYPE;
    FUNCTION ventas_por_mes(p_fecha_inicio DATE) RETURN cursor_ventas;
END analizar_ventas;
...
BEGIN
    v_cursor := analizar_ventas.ventas_por_mes(TO_DATE('2024-01-01', 'YYYY-MM-DD'));
    LOOP
...
BEGIN
    v_categoria_nombre := gestionar_categorias.obtener_categoria(41);
    DBMS_OUTPUT.PUT_LINE('Nombre de la categoría: ' || v_categoria_nombre);
END;

--Procedimientos--
BEGIN
    gestionar_categorias.agregar_categoria(p_nombre => 'iPhone');
    DBMS_OUTPUT.PUT_LINE('Categoría "iPhone" agregada exitosamente.');
END;

SELECT * FROM categorias WHERE nombre = 'iPhone';
...
BEGIN
    ...
    IF :OLD.EMAIL != :NEW.EMAIL OR :OLD.TELEFONO != :NEW.TELEFONO THEN
        ...
        VALUES (
            ...
            TO_CLOB(:OLD.EMAIL || ', ' || :OLD.TELEFONO),  
            TO_CLOB(:NEW.EMAIL || ', ' || :NEW.TELEFONO),  
            :NEW.ID_CLIENTE                     
        );
    END IF;
...
BEGIN
    ...
    IF INSERTING THEN
        ...
        VALUES (
            ...
            SYSDATE,
            NULL,
            TO_CLOB(:NEW.ID_CLIENTE || ', ' || :NEW.NOMBRE || ', ' || :NEW.APELLIDO || ', ' || :NEW.EMAIL),
            :NEW.ID_CLIENTE
        );
...
BEGIN
    INSERT INTO proveedores (nombre, descripcion, ciudad, telefono)
...
BEGIN
    INSERT INTO pedidos (id_proveedor, fecha_pedido, total_unidades, id_producto)
    VALUES (p_id_proveedor, p_fecha_pedido, p_total_unidades, p_id_producto);
END;
...
BEGIN
    OPEN p_cursor FOR
        SELECT id_proveedor, nombre, descripcion, ciudad, telefono
        FROM proveedores;
...
-- CRUD tiendas
-- Leer
CREATE OR REPLACE PROCEDURE listar_tiendas(p_cursor OUT SYS_REFCURSOR) AS
BEGIN
    OPEN p_cursor FOR
        SELECT id_tienda, nombre, direccion, ciudad
        FROM tiendas;
END listar_tiendas;
...
-- Eliminar
CREATE OR REPLACE PROCEDURE eliminar_tienda(
    p_id_tienda IN NUMBER
) AS
BEGIN
    DELETE FROM tiendas
...
-- CRUD pedidos
-- Leer
CREATE OR REPLACE PROCEDURE listar_pedidos(p_cursor OUT SYS_REFCURSOR) AS
BEGIN
    OPEN p_cursor FOR
        SELECT id_pedido, id_proveedor, fecha_pedido, total_unidades, id_producto, id_tienda
        FROM pedidos;
END listar_pedidos;
...
BEGIN
    INSERT INTO pedidos (id_proveedor, fecha_pedido, total_unidades, id_producto, id_tienda)
    VALUES (p_id_proveedor, p_fecha_pedido, p_total_unidades, p_id_producto, p_id_tienda);
END agregar_pedido;
...
-- CRUD inventarios
-- Leer
CREATE OR REPLACE PROCEDURE listar_inventarios(p_cursor OUT SYS_REFCURSOR) AS
BEGIN
    OPEN p_cursor FOR
        SELECT id_inventario, total_productos, id_tienda
        FROM inventarios;
END listar_inventarios;
...
BEGIN
    INSERT INTO inventarios (total_productos, id_tienda)
    VALUES (p_total_productos, p_id_tienda);
...
BEGIN
    UPDATE inventarios
    SET total_productos = p_total_productos,
        id_tienda = p_id_tienda       
    WHERE id_inventario = p_id_inventario;
...
BEGIN
    DELETE FROM empleados WHERE id_empleados = p_id_empleados;
    COMMIT;
...
BEGIN
    OPEN p_cursor FOR
    SELECT id_empleados, id_tienda, nombre, apellido, puesto FROM empleados;
...
BEGIN
    INSERT INTO productos (nombre, id_categoria, precio_producto, stock_producto, descripcion, id_proveedor)
    VALUES (p_nombre, p_id_categoria, p_precio_producto, p_stock_producto, p_descripcion, p_id_proveedor);
END;

create or replace PROCEDURE eliminar_producto(p_id_producto IN NUMBER) AS
BEGIN
...
BEGIN
    OPEN p_cursor FOR
        SELECT id_producto, id_categoria, nombre, precio_producto, stock_producto, descripcion, id_proveedor
        FROM productos;
END;
...
create or replace PROCEDURE eliminar_venta(p_id_ventas IN NUMBER) AS
BEGIN
...
BEGIN
    OPEN p_cursor FOR
        SELECT id_ventas, id_cliente, id_empleado, id_producto, id_tienda, fecha, cantidad, total
        FROM ventas;
END;
